...
## 12. 커밋·푸시 및 이력 문서 원본 동기화 (2026-08-29 · 2026-09-16)

### 12.1 §11 변경분 커밋·푸시 (2026-08-29)
* **시크릿 게이트 4종 전부 PASS** — ① 추적 파일 패턴 스캔 출력 없음 · ② 스테이징 추가 라인 적중 `0` · ③ `.env*` 추적 없음 · ④ `.env*` 추가 이력 없음.
* 스테이징 대상 2개만 확인 후 커밋 — `installer.iss`(버전 표기 정정), 이 문서(§11). 빌드 산출물(`dist/`·`Output/`·`build/`)은 `.gitignore` 로 제외됨.
* 커밋 `ca69c61` (2026-08-29 20:12) *build: v1.2.0 재빌드·재설치 및 설치본 버전 표기 정정* → `3c58997..ca69c61 main -> main` 푸시. 로컬 = 원격 확인.
* 사용자 확인: 재설치한 v1.2.0 이 「잘 작동한다」.

### 12.2 이 문서의 원본 동기화 (2026-09-16)
* Y드라이브 원본 폴더의 이 문서는 **2026-06-09 기록(116줄)에서 멈춰 있었다.** 2026-08-24 이후 기록은 C드라이브 작업 사본에만 있었다.
* 작업 사본의 앞 116줄이 원본과 **완전히 동일함을 diff 로 확인**한 뒤, 사본의 2026-08-24 ~ §11 기록을 그대로 이어 붙이고 이 §12 를 추가했다(CRLF → LF 로 원본 형식에 맞춤).
* ⚠ **문서만 동기화했다. Y드라이브의 소스·빌드는 여전히 구버전이다** — `main.py`·`ui/editor.py` 2026-05-28, `core/capture.py` 2026-04-23, `installer.iss` 2026-04-24, `dist/DannyCapture_Single.exe` 2026-06-09. **2026-08-24 이후 절에서 설명하는 코드(동결 캡처·자르기·재캡처·뮤텍스 등)는 이 폴더의 소스에 없다.** 최신 소스는 GitHub `bignine99/danny-capture`(`ca69c61`)와 C드라이브 사본에 있다.
* §12 는 **이 원본 문서에만 있다**(작업 사본·GitHub 은 §11 까지). 원본을 git 으로 동기화할 때 이 문서가 충돌할 수 있으니 병합에 주의한다.

### 12.3 작업 중 발생한 오류

#### 이어받기 시 최종 작업일을 6월 9일로 잘못 보고
* **오류현상**: 세션 시작 시 원본 폴더의 이력 문서만 읽고 「마지막 작업은 6/9」라고 보고함. 사용자가 「10일 이내에 C드라이브 어딘가에서 수정했다」고 정정.
* **오류원인분석**: 2026-08-24 작업이 원본이 아닌 **C드라이브 사본**(`Desktop\Temp\02 program\...`)에서 진행되었고, 이력도 사본에만 기록됨. 게다가 사본 폴더가 그 뒤 사라져 파일 탐색으로도 드러나지 않았음. 원본 쪽 `git log` 는 아래 소유권 오류로 확인 불가였음.
* **해결및개선방법**: Claude 세션 기록 디렉터리명(`C--Users-cho-Desktop-Temp-02-program-...`)에서 당시 작업 경로를 역추적 → 세션 로그에서 GitHub 저장소 생성·v1.2.0 릴리스를 확인 → 같은 경로로 clone 복원. 로그에 삭제·이동 명령은 없었음(사본 소실 원인은 [HUMAN CHECK] — 사용자 수동 삭제로 추정, 미확인). **재발 방지 — 원본과 작업 사본을 따로 두지 않는다. 불가피하면 이력 문서는 반드시 원본에도 반영한다.**

#### 네트워크 공유 원본에서 git 명령 거부
* **오류현상**: `Y:\...` 에서 `git log`·`git status` 가 `fatal: detected dubious ownership in repository` 로 실패.
* **오류원인분석**: Y드라이브는 네트워크 공유(`//192.168.0.161/...`)이고 저장소 소유자 SID 가 현재 로컬 사용자와 달라, git 의 safe.directory 보호가 작동.
* **해결및개선방법**: 금회는 우회만 함 — 파일 수정 시각·크기 비교로 상태를 대조하고, git 작업은 C드라이브 clone 에서 수행. 근본 해결(`git config --global --add safe.directory …` 등록)은 사용자 승인 대기 — TODO 1.

#### 뮤텍스 검증 직후 프로세스가 4개 — 중복 실행 오판 위험
* **오류현상**: 설치본 실행 후 2번째 실행을 시도하자 스크립트는 「2번째 인스턴스 차단·종료」라고 판정했으나, 프로세스 목록에는 `DannyCapture_Single` 이 4개 남아 있어 차단 실패처럼 보였음.
* **오류원인분석**: PyInstaller onefile 은 **인스턴스 1개당 부트로더(부모) + 실제 앱(자식) 2프로세스**다. `Start-Process` 가 돌려준 PID 는 부트로더라 먼저 끝났고, 자식은 뮤텍스에 걸려 **「이미 실행 중」 안내창을 띄운 채 사용자 확인을 기다리는 중**이었음(부모-자식 관계·창 제목 `Danny Capture` 로 확인).
* **해결및개선방법**: 안내창을 `CloseMainWindow()` 로 닫자 해당 프로세스 종료 → 1개 인스턴스(2프로세스)만 남음을 확인. **교훈 — onefile 앱은 프로세스 개수로 인스턴스를 세지 않는다. 부모 PID 와 창 제목으로 판정한다.** 같은 이유로 §11.3 의 「구버전 4프로세스」는 2인스턴스였다.

#### C드라이브 전체 `.py` 재귀 검색 타임아웃
* **오류현상**: 최근 수정 `.py` 를 사용자 폴더 전체에서 찾는 PowerShell 명령이 120초 제한을 넘겨 백그라운드로 밀린 뒤 실패.
* **오류원인분석**: `C:\Users\cho` 아래 파일 수가 많아 조건 필터 전에 전량 열거가 발생.
* **해결및개선방법**: 폴더 **이름** 패턴(`apture|캡처|캡쳐`)을 `-Depth 4` 로 제한해 검색 → 수 초 내에 사본 경로 단서(세션 기록 디렉터리)를 찾음. 넓은 파일 검색보다 이름 단서 → 좁은 확인 순서가 빠르다.

#### PowerShell 에서 `git clone` 이 오류로 표시됨
* **오류현상**: clone 실행 결과가 빨간 `NativeCommandError` 로 출력됨.
* **오류원인분석**: git 은 진행 메시지(`Cloning into …`)를 stderr 로 내보내고, Windows PowerShell 5.1 은 네이티브 명령의 stderr 를 오류 레코드로 감쌈. 실제 실패가 아님.
* **해결및개선방법**: 출력 모양이 아니라 결과로 판정 — 파일 목록(34개)·`git log`·`git describe`(`v1.2.0-1-g3c58997`) 확인 후 성공 처리.

### 12.4 TODO 갱신 (다음 세션)

| 항목 | 상태 |
|---|---|
| 1. **Y드라이브 원본 소스를 v1.2.0(`ca69c61`)으로 동기화** — safe.directory 등록 후 pull 또는 교체. 이 문서의 §12(원본에만 있음) 병합 주의 | **최우선 · 미착수** |
| 2. 2026-06-09 §6-1 설치 시 구버전 자동 종료 | 부분 해결 — v1.2.0 부터 `AppMutex` 로 감지. **뮤텍스가 없는 구버전(≤6/9)은 감지 불가**, 수동 종료 필요 |
| 3. 2026-06-09 §6-3 단축키 중복 등록 알림 | **완료** (2026-08-24 §4.2 · §5) |
| 4. 2026-06-09 §6-2 메뉴·팝업 한국어 통일 | 미착수 |
| 5. 2026-06-09 §6-4 저장 파일명 패턴 설정 | 미착수 |
| 6. 2026-08-24 §8-3 개선 후보(모자이크/블러·핀 고정·OCR·저장 폴더 열기·fit-to-window) | 미착수 |
| 7. [HUMAN CHECK] 다중 모니터 배율 125%/150% 환경 검증 (2026-08-24 §6) | 미검증 |
| 8. [HUMAN CHECK] 설치본에서 드롭다운 동결 캡처·자르기·재캡처 손 검증 | 사용자 「잘 작동한다」 확인, 항목별 확인은 미기록 |
